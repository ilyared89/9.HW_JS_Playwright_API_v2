...
test.describe('GET Challenges', () => {
  ...
  test('@GET @challenge02 получить список challenges [Challenge 02]', async ({ todoServices }) => {
  ...
  test('@GET @challenge25 получить todos в формате XML [Challenge 25]', async ({ request, token }) => {
    const response = await request.get('https://apichallenges.eviltester.com/todos', {
      headers: {
        'X-CHALLENGER': token,
  ...
  test('@GET @challenge26 получить todos в формате JSON [Challenge 26]', async ({ request, token }) => {
    const response = await request.get('https://apichallenges.eviltester.com/todos', {
      headers: {
        'X-CHALLENGER': token,
  ...
  test('@GET @challenge28 предпочтительный формат XML через Accept header (200) [Challenge 28]', async ({ request, token }) => {
...
test.describe('POST Challenges', () => {
  ...
  test('@POST @challenge01 создать challenger сессию [Challenge 01]', async ({ request }) => {
    const response = await request.post('https://apichallenges.eviltester.com/challenger', {
  ...
  test('@POST @challenge18 обновить несуществующий todo (404) [Challenge 18]', async ({ todoServices }) => {
    ...
    expect(response.errorMessages || response.error || response.status === 404).toBeTruthy();
  });
  ...
  test('@POST @challenge33 неподдерживаемый Content-Type (415) [Challenge 33]', async ({ request, token }) => {
    const response = await request.post('https://apichallenges.eviltester.com/todos', {
      headers: {
        'X-CHALLENGER': token,
...
test.describe('PUT Challenges', () => {
  ...
  test('@PUT @challenge20 частичное обновление todo (только title) [Challenge 20]', async ({ todoServices }) => {
    ...
    const { status, body } = await todoServices.fullUpdateTodoWithStatus(id, updatePayload);
  ...
  test('@PUT @challenge21 обновление без title (400) [Challenge 21]', async ({ todoServices }) => {
    ...
    const { status } = await todoServices.fullUpdateTodoWithStatus(id, updatePayload);
...
test.describe('PATCH Challenges', () => {
  ...
  test('@PATCH @challenge42 PATCH heartbeat возвращает 500 [Challenge 42]', async ({ todoServices }) => {
...
test.describe('DELETE Challenges', () => {
  ...
  test('@DELETE @challenge41 DELETE heartbeat возвращает 405 [Challenge 41]', async ({ todoServices }) => {
...
test.describe('Authentication & Authorization Challenges', () => {
  ...
  test('@AUTH @challenge48 неверные credentials (401) [Challenge 48]', async ({ todoServices }) => {
    const { status } = await todoServices.getSecretToken('wrong', 'wrong');
    expect(status).toBe(401);
  });

  test('@AUTH @challenge49 верные credentials (201) [Challenge 49]', async ({ todoServices }) => {
    const { status, token } = await todoServices.getSecretToken('admin', 'password');
    expect(status).toBe(201);
    expect(token).toBeDefined();
  });

  test('@AUTH @challenge50 неверный X-AUTH-TOKEN (403) [Challenge 50]', async ({ todoServices }) => {
    const status = await todoServices.getSecretNoteWithStatus('invalid-token');
    expect(status).toBe(403);
  });

  test('@AUTH @challenge51 отсутствует X-AUTH-TOKEN (401) [Challenge 51]', async ({ request, token }) => {
    const response = await request.get('https://apichallenges.eviltester.com/secret/note', {
      headers: { 'X-CHALLENGER': token }
    });
    expect(response.status()).toBe(401);
  });

  test('@AUTH @challenge52 получить secret note с валидным токеном [Challenge 52]', async ({ todoServices }) => {
    const { token } = await todoServices.getSecretToken('admin', 'password');
    
    const body = await todoServices.getSecretNote(token);
    expect(body.note).toBeDefined();
  });

  test('@AUTH @challenge53 обновить secret note [Challenge 53]', async ({ todoServices }) => {
    const { token } = await todoServices.getSecretToken('admin', 'password');
    
    const body = await todoServices.postSecretNote(token, 'My new secret note');
...
test.describe('Heartbeat & Status Code Challenges', () => {
  ...
  test('@STATUS @challenge44 GET heartbeat возвращает 204 [Challenge 44]', async ({ todoServices }) => {
  ...
  test('@STATUS @challenge43 TRACE heartbeat возвращает 501 [Challenge 43]', async ({ request, token }) => {
    const response = await request.fetch('https://apichallenges.eviltester.com/heartbeat', {
      method: 'TRACE',
      headers: { 'X-CHALLENGER': token }
    });
  ...
  test('@OVERRIDE @challenge45 POST as DELETE override (405) [Challenge 45]', async ({ request, token }) => {
    const response = await request.post('https://apichallenges.eviltester.com/heartbeat', {
      headers: {
        'X-CHALLENGER': token,
...
test.describe('FINAL - Challenge 58', () => {
  test('@DELETE @challenge58 удалить все todos [Challenge 58]', async ({ todoServices, token }) => {
    ...
    console.log('\n✅ Challenge 58 completed! Check progress at:', challengeUrl);
  });
});

// Финальный отчет
test.describe('Final Report', () => {
  test('@REPORT вывести ссылку для проверки challenges', async ({ token }) => {
    const challengeUrl = `https://apichallenges.eviltester.com/gui/challenges/${token}`;
    
    console.log('\n╔════════════════════════════════════════════════════════════╗');
    console.log('║           🏆 ALL TESTS COMPLETED - CHECK PROGRESS 🏆       ║');
    console.log('╠════════════════════════════════════════════════════════════╣');
    console.log(`║  Token: ${token}                  ║`);
    console.log('╠════════════════════════════════════════════════════════════╣');
    console.log('║  📊 Verify your challenges at:                             ║');
    console.log(`║  ${challengeUrl.padEnd(58)} ║`);
    console.log('║                                                            ║');
    console.log('║  ✅ Green = Completed challenges                           ║');
    console.log('║  ⬜ White = Pending challenges                             ║');
    console.log('╚════════════════════════════════════════════════════════════╝\n');
